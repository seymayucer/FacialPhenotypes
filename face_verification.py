...
class FaceVerification:
    ...
    def load_images(self, inp_csv_file):
        ...
        for i, row in pairs.iterrows():
            ...
            path1 = "{}/{}/{}_{:04d}.jpg".format(
                self.data_dir,
                row.subject_name_s1,
                row.subject_name_s1.split("/")[1],
                int(row.img_id_s1),
            )
            path2 = "{}/{}/{}_{:04d}.jpg".format(
                self.data_dir,
                row.subject_name_s2,
                row.subject_name_s2.split("/")[1],
                int(row.img_id_s2),
            )
    ...
    def verify(self, model=None):
        ...
        tpr, fpr, accuracy, best_thresholds = self.evaluate(embeddings,
                                                            self.issame,
                                                            nrof_folds=10)
        ...
        for fold_idx, (train_set, test_set) in enumerate(
            ...
            for threshold_idx, threshold in enumerate(thresholds):
                tprs[fold_idx, threshold_idx], fprs[fold_idx, threshold_idx], _ = \
                    self.calculate_accuracy(threshold,
                                    dist[test_set],
