...
def face_align(input_dir, output_dir, metadata, aligned_size):
    ...
    for index, row in metadata.iterrows():
        ...
        img_dir = Path(Path(input_dir) / row.NAME_ID).with_suffix('.jpg')

        img = cv2.imread(str(img_dir))

        landmark = row.values[1:11]
        ...
        dst_dir = Path(Path(output_dir) / row.NAME_ID).with_suffix('.jpg')
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Image alignment to 112 or 224 only')
    ...
    parser.add_argument(
        '--landmark_file',
        type=str,
        default='/mnt/6TB/VGGFace2/bb_landmark/loose_landmark_test.csv',
    ...
    vgg_landmark = pd.read_csv(args.landmark_file)

    face_align(args.data_dir,
               args.output_dir,
               vgg_landmark,
               aligned_size=args.image_size)
